...
const CartSlice = createSlice({
  ...
  reducers: {
    addToCart: (state, action)=> {
       ...
       const itemPresentIndex = cartData.findIndex((item)=> item.id = action.payload.id);

       if(itemPresentIndex !== -1) {
        const updatedCartData = [...cartData];
        updatedCartData[itemPresentIndex] = {
          ...updatedCartData[itemPresentIndex], quantity: updatedCartData[itemPresentIndex].quantity +1,
        }

        const total = updatedCartData.reduce((accumulator,currentItem) => 
        ...
        return {
         ...
         const finalData = [...cartData, {...action.payload, quantity: 1}]
          const total = finalData.reduce((accumulator, currentItem) =>
    ...
    removeFromCart:  (state, action) => {
      ...
      if(itemPresentIndex !== -1) {
        ...
        const total = updatedCartData.reduce((accumulator,current)=> 
